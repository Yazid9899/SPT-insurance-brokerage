...
import { getReportCases, parseReportFilters } from "@/lib/reports-service";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const body = await getReportCases(filters);
    return NextResponse.json(body);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to load report cases";
